...
export async function getServerSideProps(context) {
  ...
  try {
    ...
    return {
      props: {
        worksheet: JSON.parse(JSON.stringify(worksheet)),
        subject
      },
    };
  } catch (error) {
    ...
    return {
      props: { worksheet: null, error: "Failed to load worksheet.", subject },
    };
  }
}

export default function PracticeWorksheetPage({ worksheet, subject }) {
  ...
  const handleSubmit = () => {
    ...
    questions.forEach((q, i) => {
      if (selectedAnswers[i] === q.correctAnswer) points++;
    });
    setScore(points);
    setFinished(true);
  };
  ...
  return (
    <React.Fragment>
      <MainHeader title="Save My Exam : Worksheet Question Page" />
