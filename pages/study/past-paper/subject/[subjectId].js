...
export default function PastPapersBySubject({ subjectName, topics }) {
  return (
    <React.Fragment>
      ...
      <div className="min-h-screen py-24 px-6">
        ...
        <div className="max-w-6xl mx-auto">
          ...
          <p className="section-subtitle mt-4 max-w-3xl">
            Select a topic to open its question papers and mark schemes. Reviewed and ready
            ...
            {topics.length > 0 ? (
              topics.map((topic, idx) => (
                <Reveal key={topic.id} className="h-full" delay={(idx % 2) * 90}>
                  <Link
                    href={`/study/past-paper/${topic.id}`}
                  ...
                  >
                    ...
                    <div className="mt-3 flex items-center gap-2 text-ink-500">
                      ...
                      <span>{topic._count?.PastPaper || 0} paper(s)</span>
                    </div>
...
export async function getServerSideProps(context) {
  ...
  try {
    ...
    console.error("Error fetching past paper topics:", error);
    return {
      props: {
        subjectName: "Subject",
        topics: [],
        error: "Failed to load past paper topics.",
      },
    };
  }
}
